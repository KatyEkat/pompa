...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.querySelectorAll(".product-card").forEach((card) => {
		...
		function handleTouchMove(e) {
			...
			if (e.type === "touchmove" && Math.abs(diff) > 10) {
				e.preventDefault(); // отключить прокрутку
			}
	...
	const sizeBlocks = document.querySelectorAll(".product-card__sizes-hover");

	sizeBlocks.forEach((block) => {
		const sizes = block.querySelectorAll("span");

		const sizeLinks = sizeSelector.querySelectorAll(".size-selector__size");

		sizeLinks.forEach((link) => {
			const sizeText = link.textContent.trim();
			if (sizeText === "48" || sizeText === "50") {
				link.classList.add("disabled");
			}
		});

		const availableSizes = Array.from(sizes).filter(
			(size) => !size.classList.contains("disabled")
		);
